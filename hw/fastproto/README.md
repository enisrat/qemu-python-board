...
- Replace the `arch` to your desired arch
  - This is necessary since QEMU source code has different compile flags for QEMU-wide and target arch sources. Since we define HW we want the sources to be in the arch source set.
...
- Fill in any gaps in the source. You already have code for:
  ...
  - `NUM_GPIO_IN`, `NUM_GPIO_OUT`: pins in & out
    - Input pin state automtically appears in struct member `level_in`
...
- Copy this file to a new file inside folder __fastproto__
- Find&Replace `machinexyz` -> your_machine_name
- Fill in any gaps in the source
  - It contains examples for most common HW setups, such as CPUs, MemoryRegions, SysBusDevices, ...